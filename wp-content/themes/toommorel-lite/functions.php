...
function inkthemes_jquery_init() {
    if (!is_admin()) {
        wp_enqueue_style('coloroptions', get_template_directory_uri() . "/css/green.css", '', '', 'all');
...
function inkthemes_migrate_option() {
    if (get_option('inkthemes_options') && !get_option('inkthemes_option_migrate')) {
        $theme_options = array('inkthemes_logo', 'inkthemes_favicon', 'inkthemes_bodybg', 'inkthemes_slideimage1');
...
//     A writable uploads dir will pass this test. Again, there's no point overriding this one.
    ...
    if ((!$type || !$ext ) && !current_user_can('unfiltered_upload')) {
        return new WP_Error('wrong_file_type', __('Sorry, this file type is not permitted for security reasons.', 'toommorel-lite')); //A WP-core string..
    }
    ...
    if (preg_match('|^' . preg_quote(str_replace('\\', '/', $uploads['basedir'])) . '(.*)$|i', $file_name, $mat)) {
        ...
        if (!empty($attachment)) {
            return new WP_Error('file_exists', __('Sorry, That file already exists in the WordPress media library.', 'toommorel-lite'));
        }
        ...
        if (false === @copy($file, $new_file))
            return new WP_Error('upload_error', sprintf(__('The selected file could not be copied to %s.', 'toommorel-lite'), $uploads['path']));
