...
function maskitto_light_register_required_plugins() {
    ...
    $plugins = array(
        ...
        array(
            'name'      => __( 'SiteOrigin Widgets Bundle', 'maskitto-light' ),
...
function maskitto_light_get_attachment_id($image_url) {
    global $wpdb;
...
/* Get attachment by size */
if ( ! function_exists( 'maskitto_thumbnail' ) ) :
    function maskitto_thumbnail( $url = '', $size = 'full' ) {
        if( isset( $url ) && $url ) :
            
            $thumb = wp_get_attachment_image_src( maskitto_light_get_attachment_id($url), $size );
...
/* Remove redux ads */
function maskitto_light_removeDemoModeLink() { // Be sure to rename this function to something more unique
...
add_action('init', 'maskitto_light_removeDemoModeLink');


/* Remove redux notice */
add_action('admin_head', 'maskitto_light_hide_notice');
function maskitto_light_hide_notice() {
  echo '
    <style>
        .redux-messageredux-notice { display: none; }
        .updated.redux-message.is-dismissible.redux-notice { display: none; }
...
/* Remove redux menu */
add_action( 'admin_menu', 'maskitto_light_remove_redux_menu',12 );
function maskitto_light_remove_redux_menu() {
